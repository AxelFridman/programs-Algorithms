...
begin
	f4(x) = sin(20*x)+0.1*x#(10*x+(sin(x)*x*cos(x) + (1+ x^2)/(x-pi-1)) * 0.01*x^7 * 0.1*x^5 * 0.001*x^3)* (x-pi) * (x+pi) 
...
@bind N2 Slider(1:1:30)

# ╔═╡ db285f52-3f78-47f1-af4b-1873f6f80f59
begin
	find(x) = (Int(x>2.1)*2+ 3*Int(x>1.3)+ 4*Int(x>0)+ 5*Int(x>-1.5)+6*Int(x<-1.8))+(sin(x^2)*3)
...
# ╔═╡ c7367f7f-7aad-40a0-baca-d598ebc24efb
function f12(x,N)
	suma = 0
	for i in 1:N
		suma = suma + sin((i^2)*x)/(x^2)
	end
	return suma
end

# ╔═╡ 73579569-3fc7-45a2-99ee-1d32166bc624
@bind N3 Slider(1:1:1000000)

# ╔═╡ dd065d29-f608-41be-8863-1f7582ddfe6d
begin
	xspan12 = 0.1:0.01:2
	plot(xspan12, f12.(xspan12, N3))

end

# ╔═╡ 1bce414a-a942-4a8d-9bc2-b02177d96e2d
begin
	f131(x) = sin(2*pi*x)
	f132(x) = sin(18*pi*x)
	grilla = -2:0.01:2
	plot(grilla, f131.(grilla))
...
git-tree-sha1 = "3ad84c5b5bf893b0793116bb92eae95f2e210975"
...
git-tree-sha1 = "52ac2875348654c1c10051e12616c2134a24e371"
