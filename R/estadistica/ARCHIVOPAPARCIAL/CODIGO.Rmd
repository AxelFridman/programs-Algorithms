...
dummies <- dummyVars(Time ~ ., data = df[,cols_reg])
...
```{r}
xTon <- model.matrix(df$Time~.,df$Tonnage)[,-1]
y <- df$Time
y.test=y[test]

# Calculo LASSO
grid<- 10^seq(10,-2,length=100)
lasso.mod =glmnet (x[train ,],y[train],alpha =1, lambda =grid) #por default

fitRidge =glmnet(df$Tonnage,df$Time)
```                                                                 
