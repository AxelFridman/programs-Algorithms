...
p <- ggplot(dfnorm, aes(x=y2, y=y2, fill=y2)) + 
  geom_violin()
p
```
```{r}
...
p5 = ggplot(dfnorm, aes(x = chi)) + 
  geom_area(stat = "bin", bins=100)
p5 = ggplot(dfnorm, aes(x = shneid)) + 
  geom_area(stat = "bin", bins=100)
p5 = ggplot(dfnorm, aes(x = gamma)) + 
  geom_area(stat = "bin", bins=100)
p1 + p2 + p3+p4+p5
```
...
```{r}
c = ggplot(data=dfnorm, aes(y2)) + geom_density(x=y2)
c
```
